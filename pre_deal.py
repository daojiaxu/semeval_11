...
def get_labels_vector():
    ...
    for i in range(0, len(texts)):
        labels_tag[list_labels[i][7:16]].append(tokenize.word_tokenize(texts[i]))
    ...
    for j in range(0, len(list_labels)):
        ...
        while (i < len(labels_tag_qujian[list_labels[j][7:16]])):
            ...
            a = len(tokenize.word_tokenize(texts[j][:int(start)]))  # 起始区间
            b = len(tokenize.word_tokenize(texts[j][int(start):int(end)]))  # 范围
            c = a + b  # 终点区间
    ...
    for j in range(0, len(list_labels)):
        for length in range(0, len(tokenize.word_tokenize(texts[j]))):
    ...
    for j in range(0, len(list_labels)):
        ...
        while (i < len(labels_tag_word_qujian[list_labels[j][7:16]])):
            ...
            a = len(tokenize.word_tokenize(texts[j]))
...
def get_test_textVector():
    ...
    # texts_token = []
    # for i in range(0, len(texts)):
    #     texts_token.append(tokenize.word_tokenize(texts[i]))
...
def get_train_vector_TC():
    ...
    for i in range(0, train_articles.shape[0]):
        a = len(tokenize.word_tokenize(
            text_dict[str(train_articles['File_ID'][i])][:int(train_articles['Start_IDX'][i])]))  # 起始区间
        b = len(tokenize.word_tokenize(text_dict[str(train_articles['File_ID'][i])][
    ...
    for i in range(0, train_articles_word.shape[0]):
        a = len(tokenize.word_tokenize(text_dict[str(train_articles_word['File_ID'][i])]))
